...
import {
  ...
  handleIndexPrefixDuplicatedAlert,
  handleIndexPrefixOverlapAlert,
  isIndexDuplicated,
...
import { buildLambdaConcurrency } from "assets/js/utils";
...
const AppLogCreateS3: React.FC = () => {
  ...
  const handleIndexPrefixOverlap = (result: SweetAlertResult) => {
    if (result.isDismissed) {
      setCurrentStep(2);
    }
  };
  const handleIndexDuplicated = (result: SweetAlertResult) => {
    if (result.isDismissed) {
      setCurrentStep(2);
    ...
    try {
      ...
      if (isIndexPrefixOverlap(errorCode)) {
        handleIndexPrefixOverlapAlert(
          errorCode,
          message,
          handleIndexPrefixOverlap
        );
      }
      if (isIndexDuplicated(errorCode)) {
        handleIndexPrefixDuplicatedAlert(errorCode, handleIndexDuplicated);
      }
    }
  };
