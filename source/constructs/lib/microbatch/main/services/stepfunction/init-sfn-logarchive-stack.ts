...
export class InitStepFunctionLogArchiveStack extends Construct {
    ...
    constructor(scope: Construct, id: string, props: InitStepFunctionLogArchiveProps) {
      super(scope, id);
      ...
      const LogArchiveLambdaInvokePolicy = new iam.Policy(this, "LogArchiveLambdaInvokePolicy", {
        statements: [
      ...
      const cfnLogArchiveLambdaInvokePolicy = LogArchiveLambdaInvokePolicy.node.defaultChild as iam.CfnPolicy;
      cfnLogArchiveLambdaInvokePolicy.overrideLogicalId("LogArchiveLambdaInvokePolicy");

      const LogArchiveRWDDBPolicy = new iam.Policy(this, "LogArchiveRWDDBPolicy", {
        statements: [
          new iam.PolicyStatement({
            effect: iam.Effect.ALLOW,
            actions: [
      ...
      // Override the logical ID
      const cfnLogArchiveRWDDBPolicy = LogArchiveRWDDBPolicy.node.defaultChild as iam.CfnPolicy;
      cfnLogArchiveRWDDBPolicy.overrideLogicalId("LogArchiveRWDDBPolicy");

      const LogArchiveRWSNSPolicy = new iam.Policy(this, "LogArchiveRWSNSPolicy", {
        statements: [
          new iam.PolicyStatement({
            effect: iam.Effect.ALLOW,
            actions: [
      ...
      // Override the logical ID
      const cfnLogArchiveRWSNSPolicy= LogArchiveRWSNSPolicy.node.defaultChild as iam.CfnPolicy;
      ...
      this.LogArchiveRole.addManagedPolicy(microBatchIAMStack.KMSPublicAccessPolicy);
      ...
      // Override the logical ID
      const cfnLogArchiveRole = this.LogArchiveRole.node.defaultChild as iam.CfnRole;
      ...
      const S3MigrationTaskFromHistoryToArchive = new tasks.LambdaInvoke(this, "Step 1: Migration S3 Objects from Delta to Archive", {
      ...
      const dropPartitionForHistoryTable = new tasks.LambdaInvoke(this, "Step 2: Batch Drop Partitions for History Data", {
        ...
        payload: sfn.TaskInput.fromObject({
          ...
          "location.$": "States.Format('{}/{}', $.metadata.s3.archivePath, $$.Execution.Name)",
      ...
      .when(sfn.Condition.booleanEquals("$.results.delta.hasObjects", true), dropPartitionForHistoryTable)
      ...
      putStepFunctionTaskToDynamoDB.next(convertStartTimeToArchiveDate).next(S3MigrationTaskFromHistoryToArchive).next(deltaHasData);
      dropPartitionForHistoryTable.next(updateStepFunctionTaskToCompleted);

      convertStartTimeToArchiveDate.addCatch(sendFailureNotification,  {resultPath: "$.errors.convertStartTimeToArchiveDate"});
      S3MigrationTaskFromHistoryToArchive.addCatch(sendFailureNotification,  {resultPath: "$.errors.S3MigrationTaskFromHistoryToArchive"});
      dropPartitionForHistoryTable.addCatch(sendFailureNotification,  {resultPath: "$.errors.dropPartitionForHistoryTable"});
      ...
      // Override the logical ID
      const cfnLogArchive = this.LogArchive.node.defaultChild as sfn.CfnStateMachine;
      ...
      const LogArchiveStartExecutionPolicy = new iam.Policy(this, "LogArchiveStartExecutionPolicy", {
        statements: [
          new iam.PolicyStatement({
            effect: iam.Effect.ALLOW,
            actions: [
      ...
      // Override the logical ID
      const cfnLogArchiveStartExecutionPolicy = LogArchiveStartExecutionPolicy.node.defaultChild as iam.CfnPolicy;
      cfnLogArchiveStartExecutionPolicy.overrideLogicalId("LogArchiveStartExecutionPolicy");

      // Create a Role for EventBridge:Scheduler
      this.LogArchiveStartExecutionRole = new iam.Role(this, "LogArchiveStartExecutionRole", {
        assumedBy: new iam.ServicePrincipal("events.amazonaws.com"),
      });

      // Override the logical ID
      const cfnLogArchiveStartExecutionRole = this.LogArchiveStartExecutionRole.node.defaultChild as iam.CfnRole;
      cfnLogArchiveStartExecutionRole.overrideLogicalId("LogArchiveStartExecutionRole");

      this.LogArchiveStartExecutionRole.attachInlinePolicy(LogArchiveStartExecutionPolicy);
