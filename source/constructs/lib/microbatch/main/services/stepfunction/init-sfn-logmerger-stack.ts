...
export class InitStepFunctionLogMergerStack extends Construct {
    ...
    constructor(scope: Construct, id: string, props: InitStepFunctionLogMergerProps) {
      super(scope, id);
      ...
      const LogMergerLambdaInvokePolicy = new iam.Policy(this, "LogMergerLambdaInvokePolicy", {
        statements: [
      ...
      const cfnLogMergerLambdaInvokePolicy = LogMergerLambdaInvokePolicy.node.defaultChild as iam.CfnPolicy;
      cfnLogMergerLambdaInvokePolicy.overrideLogicalId("LogMergerLambdaInvokePolicy");
      ...
      // Override the logical ID
      const cfnLogMergerRWDDBPolicy = LogMergerRWDDBPolicy.node.defaultChild as iam.CfnPolicy;
      ...
      // Override the logical ID
      const cfnLogMergerRWSNSPolicy= LogMergerRWSNSPolicy.node.defaultChild as iam.CfnPolicy;
      ...
      this.LogMergerRole.addManagedPolicy(microBatchIAMStack.KMSPublicAccessPolicy);
      ...
      // Override the logical ID
      const cfnLogMergerRole = this.LogMergerRole.node.defaultChild as iam.CfnRole;
      ...
      const S3MergeTaskFromDeltaToArchive = new tasks.LambdaInvoke(this, "Step 1: Merge S3 Objects from Delta to Archive", {
        lambdaFunction: microBatchLambdaStack.S3ObjectScanningStack.S3ObjectScanning,
        integrationPattern: sfn.IntegrationPattern.WAIT_FOR_TASK_TOKEN,
        payload: sfn.TaskInput.fromObject({
          "executionName.$": "$$.Execution.Name",
          "srcPath.$": "States.Format('{}/{}={}', $.metadata.s3.srcPath, $.metadata.athena.firstPartitionKey, $.results.migrationDate.date)",
          "dstPath.$": "States.Format('{}/{}/merge/{}={}', $.metadata.s3.archivePath, $$.Execution.Name, $.metadata.athena.firstPartitionKey, $.results.migrationDate.date)",
      ...
      const S3MigrationTaskFromDeltaToArchive = new tasks.LambdaInvoke(this, "Step 2: Migration S3 Objects from Delta to Archive for Backup", {
        lambdaFunction: microBatchLambdaStack.S3ObjectScanningStack.S3ObjectScanning,
        integrationPattern: sfn.IntegrationPattern.WAIT_FOR_TASK_TOKEN,
        payload: sfn.TaskInput.fromObject({
          "executionName.$": "$$.Execution.Name",
      ...
      const S3MergedMigrationTaskFromArchiveToDelta = new tasks.LambdaInvoke(this, "Step 3: Migration Merged S3 Objects from Archive to Delta", {
        lambdaFunction: microBatchLambdaStack.S3ObjectScanningStack.S3ObjectScanning,
        integrationPattern: sfn.IntegrationPattern.WAIT_FOR_TASK_TOKEN,
        payload: sfn.TaskInput.fromObject({
          "executionName.$": "$$.Execution.Name",
          "srcPath.$": "States.Format('{}/{}/merge/{}={}', $.metadata.s3.archivePath, $$.Execution.Name, $.metadata.athena.firstPartitionKey, $.results.migrationDate.date)",
      ...
      const dropPartitionForDeltaTable = new tasks.LambdaInvoke(this, "Step 5: Batch Drop Partitions before Merging", {
        ...
        payload: sfn.TaskInput.fromObject({
            ...
            "database.$": "$.metadata.athena.database",
            "tableName.$": "$.metadata.athena.tableName",
            "location.$": "States.Format('{}/{}/original', $.metadata.s3.archivePath, $$.Execution.Name)",
            "partitionPrefix.$": "States.Format('{}={}', $.metadata.athena.firstPartitionKey, $.results.migrationDate.date)",
            "workGroup": microBatchAthenaStack.microBatchAthenaWorkGroup.name,
            "outputLocation": `s3://${stagingBucket.bucketName}/athena-results`,
            "extra": {
      ...
      const sendFailureNotification = new tasks.SnsPublish(this, "Send Failure Notification" ,{
        ...
        message: sfn.TaskInput.fromObject({
          ...
          "archivePath.$": "States.Format('{}/{}/original', $.metadata.s3.archivePath, $$.Execution.Name)",
          "status": "Failed",
          "metadata.$": "$.metadata",
        }),
      });
      ...
      .when(sfn.Condition.booleanEquals("$.results.delta.hasObjects", true), S3MigrationTaskFromDeltaToArchive)
      .otherwise(updateStepFunctionTaskToCompleted);

      putStepFunctionTaskToDynamoDB.next(convertStartTimeToETLDate).next(S3MergeTaskFromDeltaToArchive).next(deltaHasData);
      S3MigrationTaskFromDeltaToArchive.next(S3MergedMigrationTaskFromArchiveToDelta).next(addPartitionForDeltaTable).next(dropPartitionForDeltaTable).next(updateStepFunctionTaskToCompleted);

      S3MergeTaskFromDeltaToArchive.addCatch(sendFailureNotification, {resultPath: "$.errors.S3MergeTaskFromDeltaToArchive"});
      S3MigrationTaskFromDeltaToArchive.addCatch(sendFailureNotification, {resultPath: "$.errors.S3MigrationTaskFromDeltaToArchive"});
      S3MergedMigrationTaskFromArchiveToDelta.addCatch(sendFailureNotification, {resultPath: "$.errors.S3MergedMigrationTaskFromArchiveToDelta"});

      addPartitionForDeltaTable.addCatch(sendFailureNotification,  {resultPath: "$.errors.addPartitionForDeltaTable"});
      dropPartitionForDeltaTable.addCatch(sendFailureNotification,  {resultPath: "$.errors.dropPartitionForDeltaTable"});
      ...
      // Override the logical ID
      const cfnLogMerger = this.LogMerger.node.defaultChild as sfn.CfnStateMachine;
      ...
      const LogMergerStartExecutionPolicy = new iam.Policy(this, "LogMergerStartExecutionPolicy", {
        statements: [
          new iam.PolicyStatement({
            effect: iam.Effect.ALLOW,
            actions: [
      ...
      // Override the logical ID
      const cfnLogMergerStartExecutionPolicy = LogMergerStartExecutionPolicy.node.defaultChild as iam.CfnPolicy;
      cfnLogMergerStartExecutionPolicy.overrideLogicalId("LogMergerStartExecutionPolicy");

      // Create a Role for EventBridge:Scheduler
      this.LogMergerStartExecutionRole = new iam.Role(this, "LogMergerStartExecutionRole", {
        assumedBy: new iam.ServicePrincipal("events.amazonaws.com"),
      });

      // Override the logical ID
      const cfnLogMergerStartExecutionRole = this.LogMergerStartExecutionRole.node.defaultChild as iam.CfnRole;
      cfnLogMergerStartExecutionRole.overrideLogicalId("LogMergerStartExecutionRole");

      this.LogMergerStartExecutionRole.attachInlinePolicy(LogMergerStartExecutionPolicy);
