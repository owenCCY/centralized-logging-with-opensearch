...
def single_agent_installation(
    ...
    flb_download_addr = "https://releases.fluentbit.io/2.2/fluent-bit-2.2.0-win64.zip"
    if "aws-solutions-assets.s3.cn-north-1.amazonaws.com.cn" in flb_download_s3_addr:
        flb_download_addr = flb_download_s3_addr + "fluent-bit-2.2.0-win64.zip"
    ...
    if platform_type == "Linux":
        ...
        parameters = {
            ...
            "sourceHash": [
                "723d7e138d19055476dc361e86e2fd57de5d2490837cde707f512a707b27ad40"
            ],
            "commands": [
                f"curl -o C:/fluent-bit-2.2.0-win64.zip {flb_download_addr}",
                ...
                "Copy-Item -Path C:/fluent-bit/etc/* -Force -Destination C:/fluent-bit-2.2.0-win64/etc -Recurse",
...
def update_instance_name(instances_info: dict, account_id: str, region: str):
    ...
    for reservation in instances_description.get("Reservations", []):
        for description in reservation.get("Instances", []):
            ...
            if instance_name and instances_info.get(description["InstanceId"]):
