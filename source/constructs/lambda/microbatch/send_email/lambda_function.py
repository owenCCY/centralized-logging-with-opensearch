...
class Parameters(ValidateParameters):
    ...
    def _required_parameter_check(self, parameters) -> None:
        ...
        self._child_parameter_lookup_check(parameters, keys=('stateMachineId', 'stateMachineName', 'stateName', 'pipelineId',
                               'tableName', 'scheduleType', 'sourceType', 'executionId', 'notification', 'archivePath', 'executionName',
                               'status',))
        ...
        for required_param in ('service', 'recipients'):
            if required_param not in self.notification.keys():
                raise ValueError(f'Missing required key: {required_param} in notification.')
    ...
    def get_state_machine_url(self, state_machine_id):
        ...
        state_machine_url = f'{AWS_DDB_META.aws_console_url}/states/home?' \
    ...
    def get_execution_url(self, execution_id):
        ...
        execution_url = f'{AWS_DDB_META.aws_console_url}/states/home?' \
...
def lambda_handler(event, _) -> Union[dict, None]:
    ...
    for record in event.get('Records', []):
        ...
        notification_in_pipeline = get_notification_from_pipeline(pipeline_id=data.get('pipelineId', ''))
        ...
        logger.info(f'Query etl logs in DynamoDB, executionName: {param.execution_name}.')
        param.logs = sorted(list(AWS_DDB_ETL_LOG.query_subtasks(execution_name=param.execution_name, parent_task_id='00000000-0000-0000-0000-000000000000')),
        ...
        if param.notification_service == 'SES':
            ...
            logger.info(f'Unsupported notification service: {param.notification_service}, nothing to do.')
...
def get_notification_from_pipeline(pipeline_id: str) -> dict:
    ...
    if pipeline_info: 
        notification = pipeline_info.get('data', {}).get('notification', default_notification)
...
def send_email_via_sns(parameters: Parameters) -> Union[dict, None]:
    ...
    contents = ['Hello', f'You are receiving this follow-up notification because you have one or more {parameters.state_machine_name} tasks {parameters.status} to execute.', '', 
                    f'- App Pipeline Id: {parameters.pipeline_id}', f'- Table Name: {parameters.table_name}', f'- Schedule Type: {parameters.schedule_type}', f'- State Machines URL: {parameters.state_machine_url}', 
                    f'- Execution URL: {parameters.execution_url}', '']

    if parameters.logs:
        logs_table = PrettyTable(field_names=['stateName', 'API', 'startTime', 'endTime', 'status'], hrules=1)
        for log in parameters.logs:
            logs_table.add_row([log['stateName'], log['API'], log['startTime'], log['endTime'], log['status']])
        
        contents.append(logs_table.get_string())
    
    if parameters.archive_path:
        contents.append('')
        contents.append('Objects will be deleted after a few days, so please check these logs as soon as possible, you can find them in the following buckets.')
        contents.append(f'S3 Bucket URL: {parameters.archive_path}')

    response = sns_client.publish(arn=parameters.topic_arn, message='\n'.join(contents), subject=f'[Notification] {parameters.state_machine_name} task FAILED to execute.')
...
def send_email_via_ses(parameters: Parameters) -> Union[dict, None]:
    ...
    msg = {'stateMachine': {'id': parameters.state_machine_id, 'name': parameters.state_machine_name,
                                 'status': parameters.status, 'url': parameters.state_machine_url},
               'execution': {'id': parameters.execution_id, 'name': parameters.execution_name,  'url': parameters.execution_url},
               'pipeline': {'id': parameters.pipeline_id, 'tableName': parameters.table_name, 'scheduleType': parameters.schedule_type, 'type': parameters.source_type},
               'archivePath': parameters.archive_path,
               'logs': parameters.logs}

    logger.info(f"Sending template email via SES, from: {parameters.source}, to: {parameters.recipients}, "
                f"template: {parameters.ses_email_template}.")
    
    response = ses_client.send_templated_email(source=parameters.source, to=parameters.recipients,
                                    template=parameters.ses_email_template, data=msg)
