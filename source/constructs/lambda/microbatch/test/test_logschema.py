...
class TestServicesLogSchema:
    ...
    def test_cloudfront(self,):
        ...
        from pipeline_resources_builder.logschema import CLOUDFRONT_RAW, CLOUDFRONT_PARQUET, CLOUDFRONT_METRICS
    ...
    def test_alb(self,):
        from utils.aws.glue.table import TableMetaData
        from pipeline_resources_builder.logschema import ALB_RAW, ALB_PARQUET, ALB_METRICS
    ...
    def test_waf(self,):
        from utils.aws.glue.table import TableMetaData
        from pipeline_resources_builder.logschema import WAF_RAW, WAF_PARQUET, WAF_METRICS
        ...
        assert json.dumps(WAF_PARQUET.statements.create) == json.dumps("""CREATE EXTERNAL TABLE IF NOT EXISTS `{database}`.`{table_name}` (`time` bigint, `timestamp` timestamp, `formatversion` int, `webaclid` string, `webaclname` string, `terminatingruleid` string, `terminatingruletype` string, `action` string, `action_fixed` string, `terminatingrulematchdetails` array<struct<`conditiontype`:string,`sensitivitylevel`:string,`location`:string,`matcheddata`:array<string>>>, `httpsourcename` string, `httpsourceid` string, `rulegrouplist` array<struct<`rulegroupid`:string,`terminatingrule`:struct<`ruleid`:string,`action`:string,`rulematchdetails`:array<struct<`conditiontype`:string,`sensitivitylevel`:string,`location`:string,`matcheddata`:array<string>>>>,`nonterminatingmatchingrules`:array<struct<`ruleid`:string,`action`:string,`rulematchdetails`:array<struct<`conditiontype`:string,`sensitivitylevel`:string,`location`:string,`matcheddata`:array<string>>>>>,`excludedrules`:string>>, `ratebasedrulelist` array<struct<`ratebasedruleid`:string,`limitkey`:string,`maxrateallowed`:int>>, `nonterminatingmatchingrules` array<struct<`ruleid`:string,`action`:string,`rulematchdetails`:array<struct<`conditiontype`:string,`sensitivitylevel`:string,`location`:string,`matcheddata`:array<string>>>,`captcharesponse`:struct<`responsecode`:string,`solvetimestamp`:string>>>, `requestheadersinserted` array<struct<`name`:string,`value`:string>>, `responsecodesent` string, `host` string, `clientip` string, `country` string, `uri` string, `args` string, `httpmethod` string, `httprequest` struct<`clientip`:string,`country`:string,`headers`:array<struct<`name`:string,`value`:string>>,`uri`:string,`args`:string,`httpversion`:string,`httpmethod`:string,`requestid`:string>, `labels` array<struct<`name`:string>>, `captcharesponse` struct<`responsecode`:string,`solvetimestamp`:string,`failureReason`:string>, `account_id` string, `region` string, `event_hour` string, `__execution_name__` string) PARTITIONED BY (`event_hour` string, `account_id` string, `region` string, `__execution_name__` string) ROW FORMAT SERDE 'org.apache.hadoop.hive.ql.io.parquet.serde.ParquetHiveSerDe'  STORED AS INPUTFORMAT 'org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat' OUTPUTFORMAT 'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat' LOCATION '{location}' ;""")        
        assert json.dumps(WAF_PARQUET.statements.insert) == json.dumps("""INSERT INTO \"{destination_database}\".\"{destination_table}\" (\"time\", \"timestamp\", \"formatversion\", \"webaclid\", \"webaclname\", \"terminatingruleid\", \"terminatingruletype\", \"action\", \"action_fixed\", \"terminatingrulematchdetails\", \"httpsourcename\", \"httpsourceid\", \"rulegrouplist\", \"ratebasedrulelist\", \"nonterminatingmatchingrules\", \"requestheadersinserted\", \"responsecodesent\", \"host\", \"clientip\", \"country\", \"uri\", \"args\", \"httpmethod\", \"httprequest\", \"labels\", \"captcharesponse\", \"account_id\", \"region\", \"event_hour\", \"__execution_name__\") SELECT \"timestamp\", CAST(FROM_UNIXTIME(\"timestamp\" / 1000) AS timestamp), \"formatversion\", \"webaclid\", SPLIT(\"webaclid\", '/')[3], \"terminatingruleid\", \"terminatingruletype\", \"action\", CASE WHEN action = 'ALLOW' AND labels != ARRAY[] THEN 'COUNT' ELSE action END, \"terminatingrulematchdetails\", \"httpsourcename\", \"httpsourceid\", \"rulegrouplist\", \"ratebasedrulelist\", \"nonterminatingmatchingrules\", \"requestheadersinserted\", \"responsecodesent\", case when filter(httprequest.headers, x -> lower(x.name) = 'host' ) = ARRAY[] then '' else filter(httprequest.headers, x -> lower(x.name) = 'host' )[1].value end, httprequest.clientip, httprequest.country, httprequest.uri, httprequest.args, httprequest.httpmethod, \"httprequest\", \"labels\", \"captcharesponse\", SPLIT(\"webaclid\", ':')[5], SPLIT(\"webaclid\", ':')[4], DATE_FORMAT(CAST(FROM_UNIXTIME(\"timestamp\" / 1000) AS timestamp), '%Y%m%d%H'), '{{}}' FROM \"{source_database}\".\"{source_table}\";""")
        assert json.dumps(WAF_PARQUET.statements.drop) == json.dumps("""DROP TABLE IF EXISTS `{database}`.`{table_name}`""")
        assert json.dumps(WAF_PARQUET.statements.aggregate) == json.dumps("""INSERT INTO \"{destination_database}\".\"{destination_table}\" (\"time\", \"timestamp\", \"formatversion\", \"webaclid\", \"webaclname\", \"terminatingruleid\", \"terminatingruletype\", \"action\", \"action_fixed\", \"terminatingrulematchdetails\", \"httpsourcename\", \"httpsourceid\", \"rulegrouplist\", \"ratebasedrulelist\", \"nonterminatingmatchingrules\", \"requestheadersinserted\", \"responsecodesent\", \"host\", \"clientip\", \"country\", \"uri\", \"args\", \"httpmethod\", \"httprequest\", \"labels\", \"captcharesponse\", \"account_id\", \"region\", \"event_hour\", \"__execution_name__\") SELECT \"timestamp\", CAST(FROM_UNIXTIME(\"timestamp\" / 1000) AS timestamp), \"formatversion\", \"webaclid\", SPLIT(\"webaclid\", '/')[3], \"terminatingruleid\", \"terminatingruletype\", \"action\", CASE WHEN action = 'ALLOW' AND labels != ARRAY[] THEN 'COUNT' ELSE action END, \"terminatingrulematchdetails\", \"httpsourcename\", \"httpsourceid\", \"rulegrouplist\", \"ratebasedrulelist\", \"nonterminatingmatchingrules\", \"requestheadersinserted\", \"responsecodesent\", case when filter(httprequest.headers, x -> lower(x.name) = 'host' ) = ARRAY[] then '' else filter(httprequest.headers, x -> lower(x.name) = 'host' )[1].value end, httprequest.clientip, httprequest.country, httprequest.uri, httprequest.args, httprequest.httpmethod, \"httprequest\", \"labels\", \"captcharesponse\", SPLIT(\"webaclid\", ':')[5], SPLIT(\"webaclid\", ':')[4], DATE_FORMAT(CAST(FROM_UNIXTIME(\"timestamp\" / 1000) AS timestamp), '%Y%m%d%H'), '{{}}' FROM \"{source_database}\".\"{source_table}\" WHERE __execution_name__ = '{execution_name}' GROUP BY \"timestamp\", CAST(FROM_UNIXTIME(\"timestamp\" / 1000) AS timestamp), \"formatversion\", \"webaclid\", SPLIT(\"webaclid\", '/')[3], \"terminatingruleid\", \"terminatingruletype\", \"action\", CASE WHEN action = 'ALLOW' AND labels != ARRAY[] THEN 'COUNT' ELSE action END, \"terminatingrulematchdetails\", \"httpsourcename\", \"httpsourceid\", \"rulegrouplist\", \"ratebasedrulelist\", \"nonterminatingmatchingrules\", \"requestheadersinserted\", \"responsecodesent\", case when filter(httprequest.headers, x -> lower(x.name) = 'host' ) = ARRAY[] then '' else filter(httprequest.headers, x -> lower(x.name) = 'host' )[1].value end, httprequest.clientip, httprequest.country, httprequest.uri, httprequest.args, httprequest.httpmethod, \"httprequest\", \"labels\", \"captcharesponse\", SPLIT(\"webaclid\", ':')[5], SPLIT(\"webaclid\", ':')[4], DATE_FORMAT(CAST(FROM_UNIXTIME(\"timestamp\" / 1000) AS timestamp), '%Y%m%d%H'), '{{}}';""")
    ...
    def test_cloudtrail(self,):
        from utils.aws.glue.table import TableMetaData
        from pipeline_resources_builder.logschema import CLOUDTRAIL_RAW, CLOUDTRAIL_PARQUET, CLOUDTRAIL_METRICS
    ...
    def test_vpcflow(self,):
        from utils.aws.glue.table import TableMetaData
        from pipeline_resources_builder.logschema import VPCFLOW_RAW, VPCFLOW_PARQUET, VPCFLOW_METRICS
        ...
        assert VPCFLOW_METRICS.partition_info == collections.OrderedDict([('event_hour', {'type': 'time', 'from': '%Y%m%d%H', 'to': '%Y%m%d00'}), ('account_id', {'type': 'retain'}), ('region', {'type': 'retain'}), ('__execution_name__', {'type': 'default', 'value': '00000000-0000-0000-0000-000000000000'})])
        assert json.dumps(VPCFLOW_METRICS.statements.create) == json.dumps("""CREATE EXTERNAL TABLE IF NOT EXISTS `{database}`.`{table_name}` (`time` bigint, `timestamp` timestamp, `account-id` string, `vpc-id` string, `subnet-id` string, `flow-direction` string, `traffic-path` int, `type` string, `action` string, `srcaddr` string, `dstaddr` string, `pkt-src-aws-service` string, `pkt-dst-aws-service` string, `protocol` bigint, `packets` bigint, `bytes` bigint, `requests` bigint, `account_id` string, `region` string, `event_hour` string, `__execution_name__` string) PARTITIONED BY (`event_hour` string, `account_id` string, `region` string, `__execution_name__` string) ROW FORMAT SERDE 'org.apache.hadoop.hive.ql.io.parquet.serde.ParquetHiveSerDe'  STORED AS INPUTFORMAT 'org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat' OUTPUTFORMAT 'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat' LOCATION '{location}' ;""")        
        ...
        assert json.dumps(VPCFLOW_METRICS.statements.drop) == json.dumps("""DROP TABLE IF EXISTS `{database}`.`{table_name}`""")
    ...
    def test_s3(self,):
        ...
        assert json.dumps(S3_RAW.statements.drop) == json.dumps("""DROP TABLE IF EXISTS `{database}`.`{table_name}`""")
    ...
    def test_ses(self,):
        ...
        assert SES_METRICS.columns == [
            ...
            {'Name': 'sum_delivery_delay_transientcommunicationfailure', 'Type': 'bigint'}, 
            {'Name': 'sum_delivery_delay_byoiphostnamelookupunavailable', 'Type': 'bigint'}, 
