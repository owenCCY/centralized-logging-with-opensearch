...
git_dist_dir="$source_template_dir"
...
# echo "cp -r $github_dir $git_dist_dir"
# cp -r $github_dir $git_dist_dir
...
echo "cp $source_template_dir/../architecture.png $dist_dir"
cp $source_template_dir/../architecture.png $dist_dir

echo "cp $source_template_dir/../CUSTOM_BUILD.md $dist_dir"
cp $source_template_dir/../CUSTOM_BUILD.md $dist_dir
...
# echo "cp $source_template_dir/../.git/bug_report.md $dist_dir"
# cp $source_template_dir/../.git/bug_report.md $dist_dir

# echo "cp $source_template_dir/../.git/feature_request.md $dist_dir"
# cp $source_template_dir/../.git/feature_request.md $dist_dir
...
echo "cp -r cdk-solution-helper $dist_dir/deployment"
cp -r cdk-solution-helper $dist_dir/deployment

echo "cp build-s3-dist.sh $dist_dir/deployment"
cp build-s3-dist.sh $dist_dir/deployment

echo "cp solution_config $dist_dir/deployment"
cp solution_config $dist_dir/deployment

echo "cp run-unit-tests.sh $dist_dir/deployment"
cp run-unit-tests.sh $dist_dir/deployment
...
find $dist_dir -iname "tests" -type d -exec rm -rf "{}" \; 2> /dev/null
find $dist_dir -iname "cdk.out" -type d -exec rm -rf "{}" \; 2> /dev/null
find $dist_dir -iname "GeoLite2-City.mmdb" -type f -exec rm -f "{}" \; 2> /dev/null
find $dist_dir -iname ".env.production" -type f -exec rm -f "{}" \; 2> /dev/null
find $dist_dir -iname ".env.development" -type f -exec rm -f "{}" \; 2> /dev/null
